...
for ($i = 0; $i < $count; $i++) {
    ...
    if (empty($error['image'])) {
        ...
        $upload = move_uploaded_file($_FILES['documents']['tmp_name'][$i], '../upload/media/'. $_FILES['documents']['name'][$i]);

        $sub_directory = 'upload/media/';

        // insert new data to menu table
        echo $sql = "INSERT INTO `media`(`name`,`extension`,`type`,`sub_directory`,`size`) VALUES ('$image_name','$extension','$type','$sub_directory','$size')";
        $db->sql($sql);
        $res = $db->getResult();
